...
module.exports = {
  run: async (client, message, args) => {
    try {
      ...
      if (!user) {
        message.channel.send(`No Nookazon information found for ${client.users.get(searchUserId).username}.  See how to add your information by doing \`!nookazon help\``);
        return;
      }
      message.channel.send(`Nookazon information for ${client.users.get(searchUserId).username}:\nWishlist: <${user.nookazon_profile_url}/wishlist>\nCatalog: <${user.nookazon_profile_url}/catalog>`);
    } catch (err) {
      throw new Error(err);
    }
  },
